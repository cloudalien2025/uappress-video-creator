...
# Cinematic Shorts (Sora-driven, prompt-first) — v2 (indent + string fixes)
#
# Adds:
...
# Streamlit config must be set before any other Streamlit calls that render UI.
...
if output_mode == "Shorts (9:16)":
    ...
    if shorts_mode == "Cinematic (Sora)":
        ...
        # Safer parse: use the first token (e.g., '7s') and map to seconds
        preset_key = (length_preset or "").split()[0].strip()
        target_seconds = {"7s": 7, "12s": 12, "18s": 18}.get(preset_key, 12)
        ...
        with c1:
            camera_rule = st.selectbox(
                ...
                [
                    "Slow dolly-in",
                    "Slow lateral drift",
                    "Locked-off (subtle micro-jitter)",
                    "Handheld (restrained)",
                ],
                index=0,
            )
        ...
        with c2:
            lighting_rule = st.selectbox(
                ...
                [
                    "Moody low-key",
                    "Neutral documentary",
                    "Harsh fluorescent (institutional)",
                    "Sodium-vapor night exterior",
                ],
                index=0,
            )
...
with st.sidebar.expander("🧹 Maintenance", expanded=False):
    if st.button("Clear Streamlit cache", use_container_width=True):
        # These are safe no-ops if Streamlit changes internals; wrapped for robustness.
        try:
            st.cache_data.clear()
        except Exception:
            pass
