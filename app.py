...
if uploaded is not None:
    try:
        ...
        _ff = vp.find_files(extract_dir)
        # Back-compat: some pipeline versions return (scripts, audios) while newer ones return (scripts, audios, subs)
        if isinstance(_ff, (list, tuple)) and len(_ff) == 3:
            scripts, audios, subs = _ff
        elif isinstance(_ff, (list, tuple)) and len(_ff) == 2:
            scripts, audios = _ff
            subs = []
        else:
            raise ValueError(
                f"video_pipeline.find_files returned unexpected value: {type(_ff).__name__} "
                f"len={len(_ff) if hasattr(_ff, '__len__') else 'na'}"
            )
        ...
        # Pairing: pass subs when supported, but keep compatibility with older signatures.
        try:
